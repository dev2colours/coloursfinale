...
export class EnterpriseProfileComponent {
  ...
  constructor(public afAuth: AngularFireAuth, public rp: ReportsService, private ts: TaskService, private is: InitialiseService, private ps: ProjectService, private pns: PersonalService, public es: EnterpriseService, public afs: AngularFirestore, location: Location, private renderer: Renderer, private element: ElementRef, private router: Router, private as: ActivatedRoute) {
    ...
    // this.userEmployee = { name: "", id: "", email: "", bus_email: "", phoneNumber: "", photoURL: "", nationalId: "", nationality: "", address: "", department: "", departmentId: "", hierarchy: "" };
  ...
  newAction(action: workItem) {
    ...
    if (action.projectId !== "") {
      let proJeRef1 = this.afs.collection('Projects').doc(action.projectId).collection<Task>('workItems');
      let proJeRef2 = this.afs.collection('Projects').doc(action.projectId).collection('Participants').doc(action.champion.id).collection<Task>('workItems');
      let proJeRef3 = this.afs.collection('Projects').doc(action.projectId).collection('enterprises').doc(action.companyId).collection<Task>('workItems');
      let proJeRef4 = this.afs.collection('Projects').doc(action.projectId).collection('enterprises').doc(action.companyId).collection('labour').doc(action.champion.id).collection<Task>('workItems');
    }
  ...
  refreshCompany() {
    ...
    this.compStaff3 = this.es.getStaff(this.compId);
