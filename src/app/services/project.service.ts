...
import { Enterprise, ParticipantData, companyChampion, Department, asset, assetInProject, scheduleRate,
   projectRole } from '../models/enterprise-model';
...
export class ProjectService {
  ...
  sections: Observable<{ id: string; no: number; name: string; projectId: string; projectName: string; companyId: string;
    companyName: string; Bills: [abridgedBill]; }[]>;
  ...
  getProjectSections(projectId) {
    this.projectSections = this.afs.collection('Projects').doc(projectId).collection('sections', ref => {
      return ref.orderBy('no', 'asc' ) }).snapshotChanges().pipe(
  ...
  getCompSections(projectId, compId) {
    this.compSections = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(compId)
    .collection<superSections>('sections', ref => { return ref.orderBy('no', 'asc') }).snapshotChanges().pipe(
      map(actions => actions.map(a => {
  ...
  getSections(projectId, compId) {
    this.sections = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(compId)
    .collection<Section>('sections', ref => { return ref.orderBy('no', 'asc') }).snapshotChanges().pipe(
      map(actions => actions.map(a => {
        const data = a.payload.doc.data() as Section;
        const id = a.payload.doc.id;
        return { id, ...data };
  ...
  getCompSubsections(projectId, compId) {
    this.compSubsections = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(compId)
    .collection<superSections>('subSections', ref => { return ref.orderBy('no', 'asc') }).snapshotChanges().pipe(
      map(actions => actions.map(a => {
        const data = a.payload.doc.data() as superSections;
        const id = a.payload.doc.id;
        return { id, ...data };
  ...
  getRates(projectId, compId) {
    this.rates = this.afs.collection('Enterprises').doc(compId).collection<rate>('Rates', ref => {
      return ref.orderBy('name', 'asc') }).snapshotChanges().pipe(
      map(actions => actions.map(a => {
  ...
  getProjectPlantReturns (companyId, projectId) {
    const plantRef = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(companyId)
    .collection<assetInProject>('plantReturns');
  ...
  getEnterpriseScheduleOfRates(companyId, projectId) {
    const plantRef = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(companyId)
    .collection<scheduleRate>('ScheduleOfRates');
  ...
  getStaffProjTasks(projId, staffId) {
    ...
    const proRef = this.afs.collection('Users').doc(staffId).collection<Project>('projects').doc(projId);
...
/*  */
  getProCompanyABOQ(projectId, compId) {
    const compRef = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(compId);
  ...
  getBillWorkItems(projectId, compId, billID) {
    const BillRef = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(compId)
    .collection<abridgedBill>('abridgedBOQ').doc(billID);
  ...
  getSectWorkItems(projectId, compId, billID) {
    const BillRef = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(compId)
    .collection('sections').doc(billID);
  ...
  getsubSectWorkItems(projectId, compId, sub) {
    const BillRef5 = this.afs.collection('Projects').doc(projectId).collection('enterprises').doc(compId).collection('sections')
    .doc(sub.sectionId).collection('subSections').doc(sub.id).collection('workItems');
  ...
  getStaffTasksActions(staffId, projId, taskId) {
    ...
    const proRef = this.afs.collection('Users').doc(staffId).collection<Project>('projects').doc(projId);
    const taskActions = proRef.collection<Task>('tasks').doc(taskId).collection<workItem>('workItems');
  ...
  getCompanyTasks(companyId, projectId) {
    ...
    const compRef = this.afs.collection('Projects').doc(projectId).collection<Enterprise>('enterprises').doc(companyId);
  ...
  addProject(user, project, company) {
    ...
    const myProRef = this.afs.collection('/Users').doc(user.id).collection('projects');
  ...
  dismissProject(userId, project) {
    ...
    const dref = this.afs.collection('Projects')
    const entRef = this.afs.collection('Enterprises').doc(project.companyId).collection('projects');
    const myProRef = this.afs.collection('/Users').doc(userId).collection('projects');
  ...
  getMyProjectTasks(projectId, myUserId) {
    this.myProjectTasks = this.afs.collection<Enterprise>('Enterprises').doc(projectId).collection<Task>('tasks', ref => {
      return ref.where('byId', '==', myUserId ) }).snapshotChanges().pipe(
      map(b => b.map(a => {
        const data = a.payload.doc.data() as Task;
        const id = a.payload.doc.id;
        return { id, ...data };
  ...
  getTasksImChamp(myUserId) {
    this.tasksImChampion = this.afs.collection('Users').doc(myUserId).collection('tasks', ref => {
      return ref.where('champion.id', '==', myUserId) }).snapshotChanges().pipe(
      map(b => b.map(a => {
        const data = a.payload.doc.data() as Task;
        const id = a.payload.doc.id;
        return { id, ...data };
